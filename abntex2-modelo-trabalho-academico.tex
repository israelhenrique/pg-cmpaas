...
\begin{resumo}
 Este trabalho apresenta a concepção e o desenvolvimento de um \textit{plugin} de edição de mapas conceituais em cursos gerenciados pelo Moodle com integração à uma plataforma de serviços de mapas conceituais, o CMPaaS. Aborda a importância do Moodle na Educação a Distância e detalha os processos e tecnologias utilizadas na construção de um \textit{plugin} para ele. Discute também como é possível integrar o Moodle com outras plataformas de serviços.
...
\begin{resumo}[Abstract]
  \begin{otherlanguage*}{english}
    This work presents the design and development of a concept map editing plugin in courses managed on Moodle with integration to a platform of Conceptual mapping services, the CMPaaS. Addresses the importance of Moodle in Education Distance and details the processes and technologies used to build a plugin
    for him. It also discusses how it is possible to integrate Moodle with other services. 
    \vspace{\onelineskip}
...
Os mapas conceituais são uma ferramenta gráfica criada na década de 70, com o
objetivo de ser uma representação do conhecimento. São muito fáceis de utilizar e possuem
ampla aplicação na educação, podendo ser usados para apresentar o conteúdo de uma
aula, de um livro ou capítulo ou ainda como método de avaliação. Por ser muito simples,
podem ser elaborados com lápis e papel, porém é possível alcançar novas aplicações ao se
utilizar recursos computacionais.

A proposta deste trabalho é justamente utilizar a computação para ampliar a apli-
cação dos mapas conceituais. Neste contexto foi pensado na criação de uma ferramenta que
permita o uso dos mapas no ensino a distância. Assim, este projeto visa o desenvolvimento
de um \textit{plugin} para o gerenciador de cursos \textit{Modular Objected-Oriented Dynamic Learning Environment} (Moodle) que introduza os mapas conceituais nos cursos existentes
nesta plataforma.

O Moodle foi escolhido para ser a plataforma de implementação deste projeto,
pois é uma plataforma \textit{open-source}, ou seja, pode ser livremente modificada, e também é
modular, permitindo que novas ferramentas sejam adicionadas a ela facilmente. Além é
claro, de ser amplamente utilizada por instituições de ensino no mundo todo. Avaliando a lista de \textit{plugins} do Moodle, foi constatado que não há nenhuma ferramenta que permita
o uso de mapas conceituais em tarefas de cursos gerenciados pela plataforma em suas
versões mais recentes.

O produto deste trabalho permitirá que estudantes dos cursos gerenciados pelo
Moodle realizem atividades com a elaboração de mapas conceituais. Adicionado a isso, o
editor de mapas que será implementado também irá possuir integração com o \textit{Conceptual Maps as a Service} (CMPaaS), uma plataforma que oferece diversos serviços de manipulação de mapas. Alguns desses serviços necessitam de conteúdo previamente existente para funcionar, como por exemplo o serviço de inferência de mapas.
Devido ao Moodle ser muito utilizado, um \textit{plugin} de elaboração de mapas conceituais implementado nele poderia gerar uma grande quantidade de conteúdo, que  os serviços da plataforma CMPaaS poderiam utilizar, por exemplo, o serviço de geração de inferências. Por esse motivo o \textit{plugin} desenvolvido nesse trabalho também será integrado ao CMPaaS.
...
O produto final deste trabalho será um \textit{plugin} que disponibiliza um editor de criação de mapas conceituais em cursos da plataforma Moodle. O \textit{plugin} utilizará a plataforma CMPaaS como solução de armazenamento dos mapas conceituais na nuvem.
...
Para alcançar o objetivo deste trabalho inicialmente foi feita uma revisão bibliográfica visando entender o que são mapas conceituais e a sua importância para a educação. Foi feito também um estudo da arquitetura do Moodle, de forma a entender o seu funcionamento e como podem ser desenvolvidos novos \textit{plugins} para adicionar novos recursos à plataforma. 

Após isto houve a realização de um estudo sobre quais tecnologias poderiam ser utilizadas no desenvolvimento de um \textit{plugin} para o Moodle com a funcionalidade de responder tarefas com a elaboração de mapas conceituais e que pudesse utilizar o repositório de mapas do CPMaaS.

Para finalizar, o \textit{plugin} foi implementado no Moodle e foram realizados testes de suas funcionalidades. 
...
O Moodle é composto por vários \textit{plugins} independentes, sendo cada um deles responsável por uma funcionalidade que a plataforma oferece. O escopo deste trabalho é o desenvolvimento de um novo \textit{plugin}. Neste projeto foi criado \textit{plugin} do tipo \textit{assignment submission plugin},que tem com função a submissão de uma tarefa. O \textit{plugin} de tarefa permite que um professor crie uma atividade, avaliativa ou não, para os estudantes realizarem.

Este tipo de \textit{plugin} é divido em três partes:
\begin{itemize}
	\item configurações, que são opções de comportamento do \textit{plugin}.
