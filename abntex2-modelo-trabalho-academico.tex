...
Para obter a lista de mapas do usuário o \textit{plugin} envia um request para o CMPaaS com o formato mostrado na figura. A API então responde com um JSON com os dados do usuário, conforme visto na figura. Este JSON contém a URI do usuário, seu username e também a lista de urls dos mapas que o usuário possui na plataforma.

A partir deste JSON o plugin elabora novas requests que serão enviadas para a API do CMPaaS, para que sejam obtidas as informações dos mapas do usuário. Estas requests possuem o formato ilustrado na figura.

O \textit{plugin} então recebe do CMPaaS um JSON contendo os metadados do mapa, que são a sua URI, o autor, o título, a questão e a sua descrição. Conforme pode ser visto na figura.
Com estas informações é criada a interface que mostra a listagem de mapas do usuário, conforme figura. Nesta interface o usuário pode visualizar as versões de um mapa listado ou apagar este mapa.
