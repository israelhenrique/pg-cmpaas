...
\begin{resumo}
 Este trabalho apresenta a concepção e o desenvolvimento de um \textit{plugin} de edição de mapas conceituais integrado ao Moodle. O \textit{plugin} utiliza o serviço de persistência de mapas da plataforma CMPaaS, que oferece diversos serviços de processamento de mapas conceituais. São detalhados ainda os processos e tecnologias usados na construção do \textit{plugin}, e a importância de disponibilizá-lo pelo Moodle (que é central na infra-estrutura de inúmeros programas de Educação a Distância). Discute-se ainda a possibilidade de integrar o Moodle a outras plataformas e serviços.
...
A proposta deste trabalho é justamente utilizar a computação para ampliar a aplicação dos mapas conceituais. Neste contexto foi pensado na criação de uma ferramenta que
