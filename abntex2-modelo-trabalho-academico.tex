...
A educação a distância é definida de diversas  formas por autores diferentes, apesar disso é possível extrair destas definições distintas um conceito de EAD. Alguns destes conceitos são:
 \begin{itemize}
 	\item Dohemem define EAD como um sistema no qual o aluno estuda sozinho através de um material que lhe é apresentado, sendo o desempenho do estudante acompanhado por um grupo de professores. Esta forma de estudo é possivel utilizando meios de comunicação de longa distância.
...
De forma resumida é possível definir a educação a distância como um método de ensino no qual estudante e professor estão separados físicamente e que utiliza tecnologias comunicação para contornar esta separação. 
...
\section{O Moodle}
O \begin{otherlanguage*}{english}\textit{Modular Object Oriented Distance Learning (Moodle)}\end{otherlanguage*} é um ambiente virtual de aprendizagem criado por Martin Dougiamas em 1999. Ele é uma aplicação \begin{otherlanguage*}{english}\textit{open-source}\end{otherlanguage*}, o que significa que ele é livre para ser instalado, utilizado, modificado e distribuido. Ele é amplamente usado por instituições em todo o mundo e possui uma grande comunidade que contribui para correção de erros e criação de novas ferramentas. \cite{Ribeiro2007}
...
O fato do Moodle ser modular permite que sejam desenvolvidos plugins que adicionam novas funcionalidades a plataforma, como é o caso do que está sendo desenvolvido neste trabalho.
...
A \autoref{fig_cmpaas} ilustra a arquiterura do CMPaaS. Podemos observar como as aplicações fornecidas pelo portal utilizam os serviços da plataforma, muitas vezes consumindo mais de um serviço que é oferecido por ela. Além disso, é ilutrado também como um portal externo pode aproveitar os serviços disponíveis no CMPaaS, como é o caso do plugin que será apresentado posteriormente neste trabalho. Ele trata-se de um editor de mapas conceituais que armazena os mapas, criados por ele, na plataforma. Assim ele utiliza dois serviços fornecidos pelo CMPaaS, o de armazenagem de mapas e o de autenticação. 
...
\begin{figure}[htb]
	...
	\end{center}
	\caption{\label{fig_cmpaas2}(a) Integração do Portal do Conhecimento com o CMPaaS. (b) Integração do CMPaaS com serviços externos.}
\end{figure}   
